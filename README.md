...
대시보드에 참고할 글 링크(네이버 블로그 등)를 넣으면, 그 글의 본문을 읽어
말투(어미·문장 리듬·독자에게 말 거는 방식)를 흉내 낸다.
- 네이버 블로그는 m.blog.naver.com/PostView.naver로 서버사이드 본문을 받아
  헤드리스 브라우저 없이 가볍게 읽는다(app/fetch_ref.py).
- 참고 글의 말투가 내장 프롬프트의 어미 지시(예: ~다체)와 충돌하면 참고 글이 이긴다.
